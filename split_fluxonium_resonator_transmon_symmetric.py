...
class BellCircuit:
    ...
    def get_C_mat_in(self):
        ...
        self.C_mat = [
            [2 * self.Cjp + m * self.Ccp, 0, -m * self.Ccp, 0],
            [0, 0.5 * (self.Cg + self.Cjp + 2 * self.Cp), 0, 0],
    ...
    def get_fluxonium_hamiltonian(self):
        ...
        # josephson term
        V_fluxonium = 2 * self.Ejp * sp.kron(self.I, self.I)

        for n in range(10):
            coeff = (-1)**n / math.factorial(2*n) / (2**(2*n))
            V_fluxonium += -2 * self.Ejp * coeff * sp.kron(self.cos_phi_op, self.phi2**(2*n))
